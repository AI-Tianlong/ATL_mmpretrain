...
class ATL_MAEPretrainHead(BaseModule):
    ...
    def patchify(self, imgs: torch.Tensor) -> torch.Tensor:
        ...
        # h = w =224 / 16 = 14
        h = w = imgs.shape[2] // p
        # (1,3,224,224) -> (1,3, 14,16, 14,16) [128,10,224,224]-->[128,10,14,16,14,16]
        ...
        x = x.reshape(shape=(imgs.shape[0], h * w, p**2 * self.in_channels)) # [128,14,14,16,16,10]-->[128,196,16*16*10]=[128,196,2560]
        return x
    ...
    def construct_target(self, target: torch.Tensor) -> torch.Tensor:
        ...
        Returns:
            torch.Tensor: Tokenized images with the shape of B x L x C (1,196,768)  # [128,196,768] 指定的patch_size和embed dim
        """
        ...
        return target  # 返回重建的目标，这个和neck的输出是一致的。
    ...
    def loss(self, pred: torch.Tensor, target: torch.Tensor,
        ...
        target = self.construct_target(target)  # 一个[]128,10,1024,1024]的图像变为 [128,196,2560]
        # loss = pred - target 除了mask计算loss. MAE的forward传进来的
        loss = self.loss_module(pred, target, mask)   #pred是neck的输出【128，196，2560】和targets做损失。mask是一个超参。
