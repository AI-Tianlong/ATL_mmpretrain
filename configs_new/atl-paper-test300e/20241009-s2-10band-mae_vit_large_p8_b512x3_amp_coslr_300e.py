...
with read_base():
    ...
    from .._base_.datasets.atl_s2_10band_mae import *  # 图像虽然是224的,但是resize成112的？那分辨率降了一倍啊！不行吧
...
train_dataloader.update(
    batch_size=224,  # batch=128*4 = 512。我扩大成4096呢？
)
...
model = dict(
    ...
    neck=dict(      # neck 是decoder
        ...
        patch_size=patch_size,
        num_patches = (img_size//patch_size)**2,  # 28*28
        in_chans=10,
        embed_dim=1024,  # 和base什么得得对上
...
optim_wrapper = dict(
    ...
    optimizer=dict(
        type=AdamW,
        lr=1.5e-4 * 512*3 / 256,  # 这里和batchsize有关系啊！！！
        betas=(0.9, 0.95),
        weight_decay=0.05),
    paramwise_cfg=dict(
        custom_keys={
